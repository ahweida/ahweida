...
public class TreeUtils {
    ...
    public static <T> List<T> buildByRecursive(List<T> list, String idName, String parentIdName, String childrenName) throws NoSuchMethodException, InvocationTargetException, IllegalAccessException {
        ...
        for (T template : list) {
            //根据反射，获取父id的值
            String pId = reflect(template.getClass(), GET + parentIdName, template).toString();
            //如果pd=0，即是首层
            if (ROOT_ID.equals(pId)) {
                //递归查询并封装子对象
                result.add(findChildren(template, list, idName, parentIdName, childrenName));
            }
        }
        return result;
    }
